...
Elemezd a megadott forrásokat és stílusukat, majd írj egy új blog posztot a megadott témáról, amely:

1. TARTALMI KÖVETELMÉNYEK:
   - Tökéletesen követi a forrásanyagok stílusát és hangnemét
   - A kért témához kapcsolódó, de új tartalmat generál
   - Hasonló hosszúságú, mint a forrásanyagok átlagos hosszúsága
   - A forrásokból vett szakszókincset és fogalmakat használja
   - Megtartja a források jellegzetes kifejezésmódját

2. BLOG-SPECIFIKUS ELEMEK:
   - Figyelemfelkeltő címet és bevezetést tartalmaz
   - Személyes hangvételű, közvetlen stílusú
   - Interaktív elemeket (kérdések, felvetések) tartalmaz
   - Gyakorlati példákkal és alkalmazásokkal színesíti a tartalmat

3. STRUKTÚRA:
   - Logikusan felépített, jól tagolt szerkezet
   - Alcímekkel és bekezdésekkel tagolt
   - Kulcspontok kiemelése
   - Konklúzió vagy call-to-action a végén

A végeredmény egy koherens, olvasóbarát blog poszt legyen, amely ötvözi a forrásanyagok szakmaiságát 
a blog műfaj közvetlenségével és olvashatóságával.
"""

# Define SEO and visual prompts
seo_messages = [
    {"role": "system", "content": "Te egy SEO és tartalomoptimalizálási szakértő vagy."},
    {"role": "user", "content": """
    Készíts SEO elemzést a blog poszt témához:
    {topic}

    Add meg:
    1. Kulcsszavak
    2. Meta leírás javaslatok
    3. Címoptimalizálási javaslatok
    4. Belső linkek javaslatai
    """}
]

visual_messages = [
    {"role": "system", "content": "Te egy vizuális tartalom és design szakértő vagy."},
    {"role": "user", "content": """
    A következő blog poszthoz készíts részletes vizuális tartalom javaslatokat:

    Téma: {topic}
    Célközönség: {target_audience}

    Kérlek, adj javaslatokat:
    1. Képek és Illusztrációk:
       - Főkép javaslat és leírás
       - Kiegészítő képek javaslatai
    2. Infografikák:
       - Témához kapcsolódó infografika javaslatok
       - Kulcspontok vizualizációs ötletei
    3. Design elemek:
       - Tördelési javaslatok
       - Kiemelések, blokkok elrendezése
    """}
]
...
class AutoContentFetcher:
    ...
    def generate_search_queries(self, topic: str) -> List[str]:
        """Generate multiple relevant search queries for the topic"""
        # Remove any quotes from the topic
        clean_topic = topic.replace('"', '').replace("'", '')
        
        # Generate various search queries
        queries = [
            f"{clean_topic} szakmai cikk",
            f"{clean_topic} blog",
            f"{clean_topic} elemzés",
            f"{clean_topic} útmutató",
            f"{clean_topic} szakértői vélemény",
            f"{clean_topic} kutatás"
        ]
    ...
    def search_web(self, query: str, num_results: int = 5) -> List[str]:
        ...
        for engine in search_engines:
            try:
                ...
                for result in soup.select(engine['result_selector']):
                    url = result.get('href')
    ...
    def _is_valid_url(self, url: str) -> bool:
        """Validate URL and check domain relevance"""
        excluded_domains = {
        ...
        try:
            ...
            # Check for content-focused domains
            content_domains = {'.org', '.edu', '.gov', 'blog.', 'news.', 'article.'}
            if any(domain in parsed.netloc for domain in content_domains):
                return True
    ...
    def extract_content(self, url: str) -> Dict:
        ...
        try:
            ...
            # Add timestamp to track when the content was fetched
    ...
    def fetch_topic_content(self, topic: str, max_urls: int = 10) -> List[Dict]:
        """Fetch content for a topic from multiple sources"""
        all_content = []
        search_queries = self.generate_search_queries(topic)
        
        for query in search_queries:
            urls = self.search_web(query, num_results=3)
            
            with ThreadPoolExecutor(max_workers=5) as executor:
                futures = [executor.submit(self.extract_content, url) 
                ...
                for future in futures:
                    try:
                        ...
                        if content:
                            self.visited_urls.add(content['source'])
                            all_content.append(content)
                    except Exception as e:
                        print(f"Error processing content: {e}")
                        continue
                        
            if len(all_content) >= max_urls:
                break
                
        return all_content[:max_urls]

def enhance_generate_article(topic: str, target_audience: str = "general", 
                           include_seo: bool = True, include_visual: bool = True):
    print(f"\n=== Generating enhanced blog post about: {topic} ===")
    
    try:
        additional_context, unique_sentences = full_retriever(topic)
        content_fetcher = AutoContentFetcher()
        print("Fetching relevant web content...")
        web_contents = content_fetcher.fetch_topic_content(topic)
        
        all_content = []
        document_sources = set()
        web_sources = set()
        success_urls = set()  # Új set a sikeres URL-ek tárolására
        ...
        for content in web_contents:
            if content and 'text' in content:
                if 'summary' in content:
                    all_content.append(content['summary'])
                sentences = sent_tokenize(content['text'])
                relevant_sentences = sentences[:10]
                all_content.extend(relevant_sentences)
                web_sources.add(content['source'])
                success_urls.add(content['source'])  # Sikeres URL-ek mentése
        
        combined_content = "\n\n".join(all_content)
        
        content_messages = [
            {"role": "system", "content": article_system_message},
            {"role": "user", "content": f"""
            Téma: {topic}
            Célközönség: {target_audience}

            Helyi és Web források kombinált tartalma:
            {combined_content}

            Kérlek, írj egy részletes blog posztot a megadott téma alapján,
            ötvözve a helyi és internetes forrásokból származó információkat.
            A poszt legyen átfogó, szakszerű és engaging.
            """}
        ]
        ...
        if include_seo:
            seo_prompt = seo_messages[1]["content"].format(topic=topic)
            current_seo_messages = [
                seo_messages[0],
                {"role": "user", "content": seo_prompt}
            ]
            seo_response = openai.chat.completions.create(
                model=MODEL,
                messages=current_seo_messages,
                temperature=0.7,
                max_tokens=1000
            )
            seo_content = seo_response.choices[0].message.content
            final_output += f"\n## SEO és Kulcsszó Elemzés\n\n{seo_content}\n"

        if include_visual:
            visual_prompt = visual_messages[1]["content"].format(
                topic=topic,
                target_audience=target_audience
            )
            current_visual_messages = [
                visual_messages[0],
                {"role": "user", "content": visual_prompt}
            ]
            visual_response = openai.chat.completions.create(
                model=MODEL,
                messages=current_visual_messages,
                temperature=0.7,
                max_tokens=800
            )
            visual_content = visual_response.choices[0].message.content
            final_output += f"\n## Vizuális Tartalom Javaslatok\n\n{visual_content}\n"
        
        # Források kiírása részletesebben
        final_output += "\n## Források\n"
        
        if document_sources:
            final_output += "\n### Felhasznált dokumentumok:\n"
            for src in sorted(document_sources):
                final_output += f"- {src}\n"
        
        if success_urls:  # A sikeres URL-ek kiírása
            final_output += "\n### Felhasznált weboldalak:\n"
            for url in sorted(success_urls):
                final_output += f"- {url}\n"
        
        if not (document_sources or success_urls):
            final_output += "\nNem voltak felhasznált források."
            
        # Debug információk hozzáadása
        final_output += "\n\n### Keresési információk:\n"
        final_output += f"- Sikeresen feldolgozott weboldalak száma: {len(success_urls)}\n"
        if not success_urls:
            final_output += "- Nem sikerült weboldalt feldolgozni\n"
        
        return final_output
        
    except Exception as e:
        error_message = f"Hiba történt a blog poszt generálása közben: {str(e)}"
        print(error_message)
...
def create_blog_ui():
    with gr.Blocks(title="AI Blog Poszt Generátor", theme=gr.themes.Soft()) as blog_interface:
        ...
        generate_btn.click(
            fn=lambda topic, audience, seo, visual: enhance_generate_article(
                topic,
