...
def load_file(file_path):
    ...
    # Specify folder path
folder_path = "/Users/binobenjamin/Documents/Jo/Doksi"
...
def full_retriever(question: str):
    ...
    vector_data = [
        (el.page_content, el.metadata.get("source", "Ismeretlen forrás"),
         el.metadata.get("page", "N/A"), el.metadata.get("line", "N/A"))
        for el in vector_retriever.invoke(question)
    ]
    
    vector_context = "\n".join(
        f"#Document: {source} (Page: {page}, Line: {line})\n{content}"
        for content, source, page, line in vector_data
    )
...
{vector_context}
    """
    return final_data, [f"{source} (Page: {page}, Line: {line})" for _, source, page, line in vector_data]
