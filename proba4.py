...
def full_retriever(question: str):
    ...
    # Gyűjtsük össze a ténylegesen felhasznált mondatokat, elkerülve az ismétléseket
    unique_sentences = list(dict.fromkeys(vector_data))  # Egyedi mondatok kiválasztása
    ...
    # A ténylegesen felhasznált mondatok referencia listája
    sentence_references = [
        f"{idx + 1}. Az felhasznált mondathoz tartozik: {source} - oldal: {page}"
        for idx, (_, source, page) in enumerate(unique_sentences)
    ]
...
def chat(message, history):
    ...
    if sentence_references:
        # Csak a pontosan felhasznált mondatok referencia listáját adja hozzá
